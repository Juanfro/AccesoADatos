...
public class MultipleDatabases {
	...
	protected static final String XUPDATE_1 = "<xu:modifications version=\"1.0\" xmlns:xu=\""
			+ XUpdateProcessor.XUPDATE_NS + "\">" + "<xu:insert-after select=\"//SPEECH/LINE[. &amp;= 'loving']\">"
			+ "<TEST>New line inserted</TEST>" + "</xu:insert-after>" + "</xu:modifications>";
	...
	private static void loadFile(Collection collection, String path) throws XMLDBException {
		...
		XMLResource document = (XMLResource) collection.createResource(path.substring(path.lastIndexOf('/')),
				"XMLResource");
	...
	private static void shutdown(Collection collection) throws XMLDBException {
		...
		DatabaseInstanceManager manager = (DatabaseInstanceManager) collection.getService("DatabaseInstanceManager",
				"1.0");
		manager.shutdown();
	}
}
